...
public class ChessMatch {
	...
	public void nextTurn() {
		...
		// se a cor do jogador for brance ele vai alterar pra preta se não ele altera
		// para branca
	}
	...
	private boolean testCheck(Color color) {
		...
		// retorna a posição do rei na cor informada
		Position kingPosition = king(color).getChessPosition().toPosition();

		// retorna a lista de oponentes da cor informada
		List<Piece> opponentPieces = piecesOnTheBoard.stream()
				.filter(x -> ((ChessPiece) x).getColor() == opponent(color)).collect(Collectors.toList());

		for (Piece p : opponentPieces) {
			// retorna um vetor com as possiblidades de movimento de cada peça da lista de
			// oponentes
			boolean[][] mat = p.possibleMoves();

			// caso encontre verdadeiro uma opção do opennete igual a posição do rei
